...
**Decision:** documenting this honestly rather than quietly patching it. The actual fix is known (swap to a current Cohere embedding model, then re-run ingestion since old and new model embeddings aren't compatible in the same vector space), but not being pursued right now. Cohere's reliability in production has been inconsistent, and this project doesn't have RAG usage heavy enough to justify continued investment in a dependency that's already shown it can silently die out from under you. The system's fallback design, not the embedding provider, is what actually earned the trust here.
